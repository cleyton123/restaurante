...
select month(ms.data_hora_entrada) as mes, 
       count(*) as num_visitas
from tb_mesa ms
group by month(ms.data_hora_entrada)
...
select ano, cliente, qtd_pedidos
from (
    select year(ms.data_hora_entrada) as ano,
           cl.nome_cliente as cliente,
           count(pd.quantidade_pedido) as qtd_pedidos,
           row_number() over (partition by year(ms.data_hora_entrada) order by count(pd.quantidade_pedido) desc) as rn
    from tb_pedido pd
    left join tb_mesa ms
    on pd.codigo_mesa = ms.codigo_mesa
    left join tb_cliente cl
